...
class ProgressTracker:
    ...
    def create_progress_table(self):
        ...
        # 计算进度信息
        completed = len([i for i in self.results.values() 
                        if i['status'] not in ['pending', 'processing', 'processing_page']])
        ...
        # 获取所有非 pending 状态的项目
        active_items = [(f, info) for f, info in self.results.items() 
                       if info['status'] != 'pending']
        
        # 获取 pending 状态的项目
        pending_items = [(f, info) for f, info in self.results.items() 
                        if info['status'] == 'pending']
        ...
        if len(pending_items) > 0:
            table.add_row(
                "...",
                "⏳",
                progress_bar,  # 使用自定义进度条
                f"还有 {len(pending_items)} 个文件等待处理",
                "",
                style="dim italic"
            )
